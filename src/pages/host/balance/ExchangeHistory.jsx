...
async function fetchExchangeHistory({ uid, startDate, endDate }) {
  ...
  const url = `${BASE_URL}/host/exchange-salary-to-coins-history${qs ? `?${qs}` : ""}`;
  ...
  if (!res.ok) {
    throw new Error(`Failed to load exchange history (${res.status})`);
  }
...
export default function ExchangeHistoryPage() {
  ...
  useEffect(() => {
    ...
    fetchExchangeHistory({ uid: appliedUID, startDate, endDate })
      .then((json) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        if (cancelled) return;
        setErrorMsg(err.message || "Couldn't load exchange history.");
        setStatus("error");
      });
